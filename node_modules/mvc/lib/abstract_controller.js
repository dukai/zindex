...
AbstractController.prototype = {
    ...
    getRawPost: function(callback){
        if(this.request.method == "POST" || this.request.method == "PUT"){
            this._getBuffer(function(msg, data){
	...
	getRawPostBuffer: function(callback){
		if(this.request.method == "POST" || this.request.method == "PUT"){
			this._getBuffer(callback);
		}else{
			callback('NOT POST', null);
		}
	},

	_getBuffer: function(callback){
		var postBuffers = [];
		var bufferLength = 0;
		this.request.addListener('data', function(chunk){
			postBuffers.push(chunk);
			bufferLength += chunk.length;
		});

		this.request.addListener('end', function(){
			var buffer = null;
			if(postBuffers.length > 0){
				if(postBuffers.length == 1){
					buffer = postBuffers[0];
				}else{
					buffer = new Buffer(bufferLength);
					for(var i = 0, pos = 0, len = postBuffers.length; i < len; i++){
						var chunk = postBuffers[i];
