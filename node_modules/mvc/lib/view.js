...
var View = function(intent){
	this.simpleRequest = intent;
    this.config = intent.config;
	this.template = null;
};
...
View.prototype = {
	renderScript: function(callback){
		...
		if(this.config.template.isCache && templateCache[templateId]){
			//TODO: return cache
			...
			fs.readFile(filename, 'utf8', function(err, viewFileContent){
				if(err){
                    ...
                    self.config.template.isCache && (templateCache[templateId] = self.template);
					self.buildTemplate(callback);
				}
			});
		}
	...
	getViewFilename: function(){
		return this.config.app_path + '/views/' + this.simpleRequest.controllerName + '/' + this.simpleRequest.actionName + '.nhtml';
	},
	...
	partial: function(filename){
		...
		var filename = this.config.app_path + '/views/' + filename;
		if(this.config.isCache && templateCache[templateId]){
			return templateCache[templateId].render({});
		}else{
			if(fs.existsSync(filename)){
				var viewFileContent = fs.readFileSync(filename, 'utf8');
